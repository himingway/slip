...
*   完整定义实例化与正则端口映射机制，包含位宽自动推导、悬空标记、参数覆盖、`localparam` 支持、tie 常量 `'0`/`'1`。
*   明确常数连接位宽约束：仅允许带位宽常数（如 `1'b0`）或全位宽常量 `'0`/`'1`，禁止无位宽常数。其中 `'0` 和 `'1` 作为自适应宽度常量，其位宽由所连接的目标端口决定，工具直接透传到生成代码，无需推导。
*   引入悬空标记 `_`。
...
*   **可靠的元编程**：提供编译期 `#for` / `#if` 展开，严格限制条件为常量或 `param`/`localparam` 标识符，避免前端的复杂求值。
...
*   **元编程**：`#for` / `#if` 条件 **仅支持纯整数常量、`param` / `localparam` 标识符的直接引用**。任何运算符或函数调用均禁止。
...
#### 7.2.1 元编程展开
*   条件仅接受整数常量、`param`、`localparam` 标识符。
*   在 AST 上替换为展开的语句。
...
*   Phase 2：元编程、实例化与正则映射、`localparam`、常数约束、悬空、`slang` 反射、位宽推导。
...
此文档整合了我们讨论的所有特性，作为 Slip 编译器的最终开发凭据。
