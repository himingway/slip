...
The conversion is unconditional. A warning is emitted for the
assignments whose meaning the conversion changes: those that read their
own previous value (a self-referencing statement such as
\code{for (...) \{ sum = sum + b; \}}, or a variable written and then read
again on the same path). Under non-blocking semantics the read observes
the pre-clock value, so such code should be restructured (or moved to a
\code{comb} block) if the updated value was intended. Standard register
idioms --- shift registers over arrays, state machines, gated counters
--- convert silently.
...
The arithmetic functions require a numeric capture (a port named \code{ch\_b} cannot be incremented; use a string function for non-numeric names). The failure is reported as a located error naming the port and instance.
...
The analyzer tracks assignments through \code{if} and \code{for} nesting within each block. A signal driven by two \code{seq}/\code{comb} blocks, or by a procedural block and a continuous assignment, produces an error.

The check is bit-range aware: two continuous assignments driving \emph{disjoint} bits of the same variable are legal, which is what unrolled code such as:
...
produces. Overlapping or whole-variable drivers are rejected.
...
The detector builds one dependency graph per module covering every \code{comb} block \emph{and} every continuous assignment, so loops that cross a block boundary are caught as well.

Within a block, blocking (last-assignment-wins) semantics are modelled: a read resolves to the most recent prior assignment of what it reads, which is what keeps the standard \emph{init-then-accumulate} idiom loop-free:

\begin{lstlisting}[language=Slip]
comb {
    y = 0;
    y = y + b;   // OK: the read resolves to the 0 just assigned
}
\end{lstlisting}

Graph nodes are (signal, index) pairs, so storage written in disjoint elements stays independent. A chain over array elements is fine, while a genuine element cycle is reported with its indices:
...
// ERROR: combinational loop detected: st[2] -> st[1] -> st[0] -> st[2]
\end{lstlisting}

If a cycle is found, a \code{SlipSemanticError} is raised.
...
\begin{lstlisting}[language=Slip]
module m (out[7:0], other[3:0]) {
    assign out = other;  // WARNING: 'out' is 8 bits, but value is 4 bits
}
\end{lstlisting}

Unsized decimal literals are range-checked against the target rather than assumed to be 32 bits, so the common \code{y = 0;} idiom stays silent while a value that does not fit warns:

\begin{lstlisting}[language=Slip]
assign out = 42;    // OK: 42 fits in 8 bits
assign out = 300;   // WARNING: value 300 does not fit in 'out' (8 bits)
\end{lstlisting}
...
\item[\texttt{signal 'name' driven by multiple drivers}]
A signal is driven by more than one source (two procedural blocks, or a block and a continuous assignment) over overlapping bits. Disjoint bit-ranges are allowed.
...
A feedback loop was found. The message lists the storage involved, with indices where relevant (\code{st[2] -> st[1] -> st[0] -> st[2]}). The graph covers every \code{comb} block and every continuous assignment in the module.
...
\item[\texttt{duplicate module definition: 'name' is defined more than once}]
Two modules share a name in the compilation unit (including files pulled in by \code{include}).

\item[\texttt{duplicate connection to port 'name' of instance 'I'}]
The same target port is connected twice in one instance.

\item[\texttt{cannot resolve target module 'name' of instance 'I'}]
The instance target is neither defined in the compilation unit nor found in the IP index. Pass \code{-ip}/\code{-f} when the module is external IP; otherwise this is a typo.

\item[\texttt{nonblocking assignment '<=' is not allowed at module level}]
\code{<=} outside a \code{seq} block. Use a \code{seq (clk) \{ ... \}} block for sequential logic, or \code{=} for a continuous assignment.

\item[\texttt{'if'/'for'/'case' is not supported at module level}]
A procedural statement at module level. Wrap it in \code{comb \{\}} or \code{seq (clk) \{\}}, or use the compile-time forms \slipinline{`if}/\slipinline{`for}.

\item[\texttt{empty concatenation '\{\}' is not allowed}]
A concatenation with no operands has no width and no SystemVerilog equivalent.

\item[\texttt{method '.name()' is not allowed in defun bodies}]
\code{defun} bodies are evaluated in a sandbox with a safe method allowlist.

\item[\texttt{regex port mapping error for port 'name' of instance 'I'}]
Replacement evaluation failed: unknown \code{\$func}, invalid backreference, or bad arguments.
...
\item[\texttt{in seq block of module 'M', the blocking assignment to 'name' reads its own previous value}]
A blocking assignment converts to nonblocking in a \code{seq} block (unconditionally, as described in the conversion chapter). This warning fires for the assignments whose meaning that changes: a read-modify-write of storage already written on the same path, such as a loop accumulator (\code{for (...) \{ sum = sum + b; \}}) or \code{y = 0; y = y + b;}. The read will observe the pre-clock value, so restructure or use a \code{comb} block if the updated value was intended. Register transfers (shift registers, \code{t = a + 1; o = t;}) convert silently.
...
\item[\texttt{value N does not fit in 'name' (M bits)}]
An unsized decimal literal is outside the target's range (e.g.\ \code{out = 300} for an 8-bit \code{out}).
...
\item[\texttt{unconnected output port 'name' in instance 'I' of module 'M'}]
An output port matched no connection and no regex rule; it is left dangling.

\item[\texttt{duplicate module 'name' in IP sources}]
Two IP files define the same module. The first definition is kept; the choice is otherwise arbitrary.

\item[\texttt{output directory ... contains SystemVerilog files not produced by this run}]
Stale \code{.sv} files from an earlier build (a renamed module, say) are still present. Downstream tools that compile a whole directory would pick them up.
