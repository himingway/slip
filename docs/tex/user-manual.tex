...
\lstdefinelanguage{Slip}{
  keywords={module,param,localparam,logic,signed,assign,seq,comb,initial,
            pos,neg,if,else,for,case,casez,casex,default,inside,begin,end},
  sensitive=true,
  morecomment=[l]{//},
  morecomment=[s]{/*}{*/},
  morestring=[b]",
...
\begin{enumerate}
  \item Slip scans IP directories for \code{.sv} and \code{.v} files.
...
\item[\texttt{cannot override localparam 'name'}]
Attempting to override a localparam in instance \code{\#(...)}.

\item[\texttt{unconnected input port 'name'}]
Input port not connected and not marked with \code{\_}.
